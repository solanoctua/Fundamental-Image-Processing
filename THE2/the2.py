...
def dft(image):
    """
    The FFT algorithm is significantly faster than the direct implementation.
    However, it still lags behind the numpy implementation by quite a bit.
    One reason for this is the fact that the numpy implementation uses matrix operations
    to calculate the Fourier Transforms simultaneously.
    """
    ...
    print("v: ",v.shape)
    print(u*v)
    F = np.exp(-2j * np.pi * ((u * v / M) + (u * v / N)))
    print("F: ",F.shape)
    return np.dot(F, image)
...
def discrete_Fourier_Transform(image ):
    ...
    for x in range(0, number_of_rows):
        for y in range(0, number_of_columns):
            ...
            print("F({},{}) = {}".format(x,y,transformed[x][y]))
            #print(transformed[x][y])
    
def part1(input_img_path, output_img_path):
    ...
    create_output_path(output_img_path)
    ...
    #discrete_Fourier_Transform(img)
    """
    transformed = np.uint8(dft(img))
    print("transformed.shape:",transformed.shape)
    """
    
    #fourier = cv2.dft(img.astype(np.float32), flags=cv2.DFT_COMPLEX_OUTPUT)
    
    # 2 dimensional Fourier Transform
    #fourier = dft(img)
    #fourier = discrete_Fourier_Transform(img)
    """
    using recursive approach (odd-even) reduces the time complexity of 2d DFT from O(n^2*m^2) to 0(nm*log(nm)) where nxm is the size of image
    ...
    np.savetxt("olalla.txt",fourier)
    cv2.imwrite("DiscreteFourierTransform_shifted.png", transformed)

    """
    lowpass_filter = np.ones((number_of_rows, number_of_columns), np.uint8)
    radius = 100  #cutoff frequency
    ...
    filtered = transformed * highPass_filter
    shifted = shifted*highPass_filter
    """
    """
    shifted = highPassFilter(shifted, 15)
    magnitude = 20*np.log(np.abs(shifted)) #only for visualizing purposes 
    transformed = np.asarray(magnitude, np.uint8)
    cv2.imwrite("IdealHighPass_Fourier.png", transformed)

    # Apply inverse shift
    reshifted = np.fft.ifftshift(shifted)
    ...
    magnitude = 20*np.log(np.abs(Gaussian)) #only for visualizing purposes 
    frequencykernel = np.asarray(magnitude, np.uint8)
    cv2.imshow("Gaussian Highpass Filter",frequencykernel)
    cv2.imwrite("GaussianHighpassFilter.png", frequencykernel)
    
    filtered = Gaussian * shifted
    
    magnitude = 20*np.log(np.abs(filtered)) #only for visualizing purposes 
    frequencykernel = np.asarray(magnitude, np.uint8)
    cv2.imshow("Gaussian Highpass Applied",frequencykernel)
    cv2.imwrite("GaussianHighpass_Frequency.png", frequencykernel)
    
    # Apply inverse shift
    reshifted = np.fft.fftshift(filtered)
    # Return to spatial domain using Inverse Fourier Transformation
    inversefourier = np.fft.ifft2(reshifted)
    inversefourier = np.abs(inversefourier)
    ...
    spatial = inversefourier.astype(np.uint8)
    cv2.imshow("Gaussian",spatial)
    cv2.imwrite("GaussianHighpass_Spatial.png", spatial)
    ...
    laplacian = np.array([[1, 4, 1],
                        [4, -20, 4],
                        [1, 4, 1]])
    sobel_combined = laplacian
    """
    padding = (number_of_rows - 3, number_of_columns - 3)  # total amount of padding
    sobel_horizontal = np.pad(sobel_horizontal, (((padding[0]+1)//2, padding[0]//2), ((padding[1]+1)//2, padding[1]//2)), 'constant')
    sobel_vertical = np.pad(sobel_vertical, (((padding[0]+1)//2, padding[0]//2), ((padding[1]+1)//2, padding[1]//2)), 'constant')
    sobel_combined = np.pad(sobel_combined, (((padding[0]+1)//2, padding[0]//2), ((padding[1]+1)//2, padding[1]//2)), 'constant')
    
    """
    sobel_horizontal = np.zeros((number_of_rows, number_of_columns), np.uint8)
    sobel_horizontal[239:249,239:249] = 1
    sobel_horizontal = np.fft.fft2(sobel_horizontal)
    sobel_horizontal = np.fft.fftshift(sobel_horizontal)

    sobel_vertical = np.zeros((number_of_rows, number_of_columns), np.uint8)
    sobel_vertical[0:40,0:20] = 1
    sobel_vertical = np.fft.fft2(sobel_vertical)
    sobel_vertical = np.fft.fftshift(sobel_vertical)
    """

    """
    filtered = shifted*sobel_horizontal
    #filtered = shifted*sobel_vertical
    magnitude = 20*np.log(np.abs(filtered)) #only for visualizing purposes 
    frequencyfiltered = np.asarray(magnitude, np.uint8)
    cv2.imshow("Sobel applied",frequencyfiltered)
    """
    
    sobel_horizontal = np.fft.fft2(sobel_horizontal)
    sobel_horizontal = np.fft.fftshift(sobel_horizontal)
    
    magnitude_sobel_horizontal = 20*np.log(np.abs(sobel_horizontal)) #only for visualizing purposes 
    frequency_sobel_horizontal = np.asarray(magnitude_sobel_horizontal, np.uint8)
    cv2.imshow("Sobel Horizontal Filter",frequency_sobel_horizontal)
    ...
    magnitude_sobel_vertical = 20*np.log(np.abs(sobel_vertical)) #only for visualizing purposes 
    frequency_sobel_vertical = np.asarray(magnitude_sobel_vertical, np.uint8)
    cv2.imshow("Sobel Vertical Filter",frequency_sobel_vertical)
    ...
    magnitude_sobel_combined = 20*np.log(np.abs(sobel_combined)) #only for visualizing purposes 
    frequency_sobel_combined = np.asarray(magnitude_sobel_combined, np.uint8)
    cv2.imshow("Sobel Vertical Filter",frequency_sobel_combined)
    #cv2.imwrite("SobelCombinedFilter.png", frequency_sobel_combined)
    
    
    #filtered = filtered*sobel_horizontal
    #filtered = filtered*sobel_vertical
    ...
    for i in range(1, number_of_rows - 1):
        for j in range(1, number_of_columns - 1):
            ...
            neighbors = (spatial[i-1:i+2,j-1:j+2]).tolist()
            #print(neighbors)
            if (all(spatial[i][j]*(1) >= value for value in neighbors[0])):
                thresholded[i][j] = spatial[i][j]
            else:
                count +=1
                thresholded[i][j] = 0
    ...
    #edge_map = np.zeros((number_of_rows,number_of_columns), np.uint8)
    
    cv2.imshow("Original Image",img)
    cv2.imshow("Frequency Domain ", transformed)
    #cv2.imshow("Filtered", filtered)
    
    #cv2.imshow("Back to Spatial Domain", spatial)
    
    #result = np.concatenate((spatial,thresholded),1)
    cv2.imshow("Back to Spatial Domain", spatial)
    cv2.imwrite("Edge.png", spatial)
    #cv2.imwrite(output_path + "edges.png", edge_map)
...
def boxFilter(image, targetpixel, size):
    ...
    #print(targetpixel,size)
    #print("Boxfiltering ({},{}) from ({},{}) to ({},{})".format(targetpixel[0],targetpixel[1], targetpixel[0] - ((size[0]-1)//2), targetpixel[1]-((size[1]-1)//2), targetpixel[0]+((size[0]-1)//2),targetpixel[1]+1+((size[1]-1)//2)))
    boxFilter[targetpixel[0]-((size[0]-1)//2) : targetpixel[0]+((size[0]-1)//2),targetpixel[1]-((size[1]-1)//2) : targetpixel[1]+1+((size[1]-1)//2)] = 0
    #print(boxFilter[targetpixel[0]][targetpixel[1]])
...
def enhance_3(path_to_3, output_path):
    ...
    #img = cv2.resize(R, (720,720), interpolation= cv2.INTER_LINEAR)
    ...
    for i in range (20,60):
        if (i%2 != 0):
            Rfiltered = bandRejectFilter(Rfiltered, (0+1*i,0+1*(i+1)))
    """
    """
    inner = 30
    outer = 150
    ...
    x, y = np.ogrid[ :number_of_rows, :number_of_columns]
    band_row, band_column = np.where(np.logical_and((x - x_center)**2 + (y - y_center )**2 <= outer**2 ,(x - x_center)**2 + (y - y_center )**2 >= inner**2 ))
    Rfiltered[band_row, band_column] = 0
    """
    ...
    x, y = np.ogrid[ :number_of_rows, :number_of_columns]
    circle_row, circle_column = np.where((x - x_center)**2 + (y - y_center )**2 <= radius**2 )
    #Bshifted[circle_row, circle_column] = 0

    Bfiltered = passFilter(Bshifted,"Lowpass", 120)
    #Bfiltered = bandRejectFilter(Bfiltered, (14,18))
    """
    count = 0
    print("this",int(Bfiltered[684][768]))
    for i in range(0,number_of_rows):
        for j in range (0,number_of_columns):
            if ((i - number_of_rows//2)**2 + (j - number_of_columns//2 )**2 <= 150**2 and Bfiltered[i][j] <= 0):
                count += 1
                Bfiltered[i][j] = 100
            else:
                continue
    ...
    for i in range (18,60):
        if (i%2 != 0):
            Bfiltered = bandRejectFilter(Bfiltered, (0+1*2*i,0+2*(i+1)))
    """
    # GREEN CHANNEL
    ...
    for i in range (20,60):
        if (i%5 == 0):
            Gfiltered = bandRejectFilter(Gfiltered, (i,i+1))
    """
    ...
    cv2.imwrite(output_path+"/3EnhancedB.png", Benhanced)
...
def enhance_4(path_to_4, output_path):
    ...
    Bfiltered = boxFilter(Bfiltered,(number_of_rows//2,number_of_columns//4),(4,number_of_rows//2 -15 )) # left horizontal line
    Bfiltered = boxFilter(Bfiltered,(number_of_rows//4,number_of_columns//2),(number_of_columns//2 -15,4))  # upper vertical line
    Bfiltered = boxFilter(Bfiltered,(int(number_of_rows*3/4),number_of_columns//2),(number_of_columns//2 -15  ,4))  # bottom vertical line
    Bfiltered = boxFilter(Bfiltered,(number_of_rows//2,int(number_of_columns*3/4)),(4,number_of_rows//2 -15))  # right horizontal line
    
    Gfiltered = passFilter(Gshifted,"Lowpass", 200)
    #Gfiltered = bandRejectFilter(Gfiltered, (20,40))
    Gfiltered = boxFilter(Gfiltered,(number_of_rows//2,number_of_columns//4),(4,number_of_rows//2 -15))  # left horizontal line
    Gfiltered = boxFilter(Gfiltered,(number_of_rows//4,number_of_columns//2),(number_of_columns//2 -15 ,4))  # upper vertical line
    Gfiltered = boxFilter(Gfiltered,(int(number_of_rows*3/4),number_of_columns//2),(number_of_columns//2 -15 ,4))  # bottom vertical line
    Gfiltered = boxFilter(Gfiltered,(number_of_rows//2,int(number_of_columns*3/4)),(4,number_of_rows//2 -15))  # right horizontal line
    
    Rfiltered = passFilter(Rshifted,"Lowpass", 200)
    Rfiltered = boxFilter(Rfiltered,(number_of_rows//2,number_of_columns//4),(4,number_of_rows//2 -15))  # left horizontal line
    Rfiltered = boxFilter(Rfiltered,(number_of_rows//4,number_of_columns//2),(number_of_columns//2 -15 ,4))  # upper vertical line
    Rfiltered = boxFilter(Rfiltered,(int(number_of_rows*3/4),number_of_columns//2),(number_of_columns//2 -15 ,4))  # bottom vertical line
    Rfiltered = boxFilter(Rfiltered,(number_of_rows//2,int(number_of_columns*3/4)),(4,number_of_rows//2 -15))  # right horizontal line
    ...
    cv2.imshow("All Filtered", filtered)
    """
    cv2.imwrite("4Filtered.png", filtered)
    ...
    result = np.concatenate((Benhanced,Genhanced,Renhanced),1)
    #cv2.imshow("All Back to Spatial Domain", result)
    cv2.imwrite("4Enhanced.png", enhanced)
    return enhanced
...
def BGR2YCrCb(image):
    ...
    #coefficients = [[65.481, 128.553, 24.966], [-37.797, 74.203, 112.0], [112.0, -93.786, -18.214]]
...
def YCrCb2BGR(image):
    ...
    #image is BGR
    converted = np.zeros(image.shape, np.uint8) 
    coefficients = [[1/0.299, 1/0.587, 1/0.114], [-1/0.168736, -1/0.331264, 1/0.5], [1/0.5, -1/0.418688, -1/0.081312]]
    #coefficients = [[65.481, 128.553, 24.966], [-37.797, 74.203, 112.0], [112.0, -93.786, -18.214]]
    #B
    ...
    #converted[:,:,1] =   coefficients[1][0]*image[:,:,2] + coefficients[1][1]*image[:,:,1] + coefficients[1][2]*image[:,:,0] 
    #R
    converted[:,:,2] =  image[:,:,0] + 1.772 * (image[:,:,2]-128)
    #converted[:,:,2] =   coefficients[2][0]*image[:,:,2] + coefficients[2][1]*image[:,:,1] + coefficients[2][2]*image[:,:,0]
...
def DCT(block):
    ...
    dct = np.zeros((blocksize,blocksize),np.float32 ) #np.int16
    for u in range(0,blocksize):
        for v in range(0,blocksize):
            summation = 0
            for i in range(0,blocksize):
                for j in range(0,blocksize):
                    summation += np.sqrt(2/blocksize)*block[i][j]*np.cos(np.pi*u*(2*i+1)/(2*blocksize))*np.cos(np.pi*v*(2*j+1)/(2*blocksize))
                    if(i == 0):
                        c1 = 1/np.sqrt(2)
                    else:
                        c1 = 1
...
def IDCT(block):
    blocksize = block.shape[0]
    idct = np.zeros((blocksize,blocksize),np.float32 ) #np.int16
    for i in range(0,blocksize):
        for j in range(0,blocksize):
            summation = 0
            for u in range(0,blocksize):
...
def huffmanEncoding(array):
    ...
    for coeff in range (0,len(output)):
        ...
        #print("1'st comlement of {} is {}".format(bin(output[coeff][1]),huffman[-1]))
    #print("binary output: ", huffman)
    ...
    for value in values:
        ...
        occurences.append((value,countOccurences(values,value)))
        
    
    occurences_sorted = sorted(occurences, key=lambda row: row[1])
    unrepeated = []
    #print("sorted occurences: ", occurences_sorted)
    for i in range(0,len(occurences_sorted)):
        if not(occurences_sorted[i] in unrepeated):
    
            unrepeated.append(occurences_sorted[i])
    print("unrepeated sorted occurences: ", unrepeated)
    values = [i[0] for i in unrepeated]
    probabilities = [i[1]/64 for i in unrepeated]
    print("values: ",values)
    ...
    for x in range(len(values)):
        nodes.append(node(probabilities[x], values[x]))
...
def the2_write(input_img_path, output_path):
    ...
    #print("Cr: {} Cb: {}".format(Cr.shape,Cb.shape))
...
def the2_read(input_img_path):
    ...
    Cb = image[:,:,2]
    """
    image[0:int(size[0]),0:int(size[1])][1]= np.resize(image[1][0:int(size[0])//2,0:int(size[1])//2],(int(size[0]),int(size[1])))
    image[0:int(size[0]),0:int(size[1])][2]= np.resize(image[2][0:int(size[0])//2,0:int(size[1])//2],(int(size[0]),int(size[1])))
    """
    Cr4 = Cr[0:int(size[0])//2,0:int(size[1])//2]
    Cr = cv2.resize(Cr4, (2048, 1536), interpolation= cv2.INTER_LINEAR)
    print("Cr.shape: ",Cr.shape)
    """
    Cr[0::2,0::2] = Cr4[::1,::1]#np.resize(Cr[0:int(size[0])//2,0:int(size[1])//2],(int(size[0]),int(size[1])))
    Cr[1::2,1::2] = Cr4[::1,::1]
    """
...
#create_output_path('./THE2outputs/')
"""
create_output_path('./THE2outputs/Part1/')
create_output_path( './THE2outputs/Part1/1/')
create_output_path('./THE2outputs/Part1/2/')
create_output_path('./THE2_images/compression_outputs')
"""
